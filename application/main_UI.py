...
COVER = 'D:\\final-design\PFLD-pytorch-master2\\application\images\camera.png'
...
class Fatigue_detecting(wx.Frame):
    ...
    def __init__( self, parent, title ):
        ...
        m_listBox2Choices = [ u"30", u"4", u"5", u"6", u"7", u"8" ]
    ...
    def _learning_face(self,event):
        ...
        self.predictor = dlib.shape_predictor("D:\\final-design\PFLD-pytorch-master2\\application\model\shape_predictor_68_face_landmarks.dat")
        ...
        while(self.cap.isOpened()):
                ...
                for k, d in enumerate(faces):
                    ...
                    if self.nod_checkBox7.GetValue()== True:
                        ...
                        if mar < self.MAR_THRESH:
                            if har > self.HAR_THRESH:  # 点头阈值0.5
                                self.hCOUNTER += 1
                            else:
                                # 如果连续3次都小于阈值，则表示点头一次
                                if self.hCOUNTER >= self.NOD_AR_CONSEC_FRAMES:  # 阈值：3
                                    ...
                                    self.m_textCtrl3.AppendText(
                                        time.strftime('%Y-%m-%d %H:%M ', time.localtime()) + u"点头\n")
                                # 重置点头帧计数器
                                self.hCOUNTER = 0
            ...
            # 确定疲劳提示:眨眼14次，打哈欠5次，点头6次
            if self.TOTAL >= 14 or self.mTOTAL>=5 or self.hTOTAL>=6:
